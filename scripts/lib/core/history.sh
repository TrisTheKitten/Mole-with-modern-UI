...
history_ensure_log_file() {
    local log_file="$1"
    ensure_user_dir "$(dirname "$log_file")"
    ensure_user_file "$log_file"
}

history_append_operation_line() {
    local command="$1"
    local action="$2"
    local detail="${3:-}"
    local log_file
    log_file=$(history_operations_log_file)

    history_ensure_log_file "$log_file"
    printf '[%s] [%s] %s %s\n' "$(date '+%Y-%m-%d %H:%M:%S')" "$command" "$action" "$detail" >> "$log_file" 2> /dev/null || true
}

history_append_session_start() {
    local command="$1"
    local log_file
    log_file=$(history_operations_log_file)

    history_ensure_log_file "$log_file"
    printf '# ========== %s session started at %s ==========\n' "$command" "$(date '+%Y-%m-%d %H:%M:%S')" >> "$log_file" 2> /dev/null || true
}

history_append_session_end() {
    local command="$1"
    local items="$2"
    local size="$3"
    local log_file
    ...
    history_ensure_log_file "$log_file"
    printf '# ========== %s session ended at %s, %s items, %s ==========\n' "$command" "$(date '+%Y-%m-%d %H:%M:%S')" "$items" "$size" >> "$log_file" 2> /dev/null || true
}

history_append_deletion() {
    local mode="$1"
    local size_kb="$2"
    local status="$3"
    local path="$4"
    local log_file
    log_file=$(history_deletions_log_file)

    history_ensure_log_file "$log_file"
    printf '%s\t%s\t%s\t%s\t%s\n' "$(date '+%Y-%m-%d %H:%M:%S')" "$mode" "$size_kb" "$status" "$path" >> "$log_file" 2> /dev/null || true
}
